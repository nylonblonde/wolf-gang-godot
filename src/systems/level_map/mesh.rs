...
#[allow(clippy::many_single_char_names)] pub fn create_drawing_system() -> Box<dyn FnMut(&mut World, &mut Resources)> {
    ...
    Box::new(move |world, resources| {
        ...
        let (map_mesh_tx, map_mesh_rx) = crossbeam_channel::unbounded::<(Entity, HashMap<usize, VertexData>)>();

        let (done_changes_tx, done_changes_rx) = crossbeam_channel::unbounded::<(Entity, ChangeType)>();
        ...
        let (to_change_tx, to_change_rx) = crossbeam_channel::unbounded::<(Entity, Entity, AABB, ChangeType)>();
